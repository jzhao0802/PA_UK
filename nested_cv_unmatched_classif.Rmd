...
__Hyper-parameters__ of elastic net:
...
* `tune.threshold=T` will treat the classification threshold as a hyperparameter, instead of rigidly fixing it at 0.5. This is again very useful and comes for free with mlR. See relevant discussion [here](https://github.com/mlr-org/mlr/issues/856).
* The same works with simple grid search as well, simply do: `ctrl <- makeTuneControlGrid( tune.threshold=T)`
* If we want to search over a fixed grid, we could specify it like so:
```{r}
ps <- makeParamSet(
  makeDiscreteParam("alpha", values=seq(0, 1, by=.25)),
  makeDiscreteParam("s", values=values=seq(0, 2, by=.25))
)
```
